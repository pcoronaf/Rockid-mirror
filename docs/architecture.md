...
| Extended screen via DisplayManager, not MediaProjection | A virtual display with `OWN_CONTENT_ONLY` shows only what we put on it, so it is a second screen and needs no capture consent. | Mirroring stays the MediaProjection path. |
| Extended screen content is a window, not an activity | Android refuses activity launches onto a virtual display an ordinary app created, including the app's own. A `Presentation` is the supported way to reach a second display; an overlay window is the fallback when even that is declined. | A trusted display would allow activities, but that flag is system-only. |
