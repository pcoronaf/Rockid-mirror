...
| Second display / app launching | **Confirmed refused on Android 16, for every app including our own.** `ActivityOptions.setLaunchDisplayId` onto a virtual display created by an ordinary app fails with `Permission Denial: starting Intent ... with launchDisplayId=N`, both for a third-party package and for the sender's own activity. No user-grantable permission lifts it; the display would have to be trusted, which is a system-only flag. Content therefore reaches the display as a **window**, not an activity: a `Presentation`, with an overlay window (`SYSTEM_ALERT_WINDOW`) as a second route. | measured on SM-S938B / Android 16 |
